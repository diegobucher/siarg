...
public class ReflectUtil {
  ...
  public static Method getSetterMethod(Object object, Field field) throws NoSuchMethodException {
    return object.getClass().getDeclaredMethod(
        "set" + field.getName().replaceFirst(field.getName().substring(0, 1), field.getName().substring(0, 1).toUpperCase()),
        field.getType());
  }

  public static Method getGetterMethod(Object object, String methodName) throws NoSuchMethodException {
    return object.getClass()
        .getDeclaredMethod("get" + methodName.replaceFirst(methodName.substring(0, 1), methodName.substring(0, 1).toUpperCase()));
  }
      ...
      if (arr.length > 1) {
        ...
        try {
          ...
          for (int i = 0; i < interfaces.length;) {
        	if(interfaces[i].isAssignableFrom(java.util.Collection.class)){
  ...
  /**
   * Verifica se o atributo do objeto  é  uma coleção Pega a primeira parte do attributo, ou seja, tudo antes do primeiro
   * ponto.
   * 
   */
  @SuppressWarnings({ "rawtypes" })
