...
Date.MONTH_NAMES = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];
...
function Chronoline(domElement, events, options) {
        ...
        for(var row = 0; row < t.eventRows.length; row++){
            ...
            for(var col = 0; col < t.eventRows[row].length; col++){
                ...
                if(t.tooltips){
                    ...
                    j$.extend(true, usedQtipOptions, t.qtipOptions);
    ...
    t.initDraggable = function() {
        ...
        t.stopDragging = function(e){
            j$(t.wrapper).removeClass('dragging')
        ...
        j$(t.paperElem).mousedown(function(e){
            e.preventDefault();
            t.dragMouseStart = e.pageX;
            t.dragPaperStart = getLeft(t.paperElem);
            j$(t.wrapper).addClass('dragging')
